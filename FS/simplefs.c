...
void SimpleFS_printDirBlocks (DirectoryHandle* dirhandle) {
	...
	printf ("{ ");
	iterator = dirhandle->current_block;
	int i = 0;
	while (i < dirhandle->dcb->fcb.size_in_blocks) {
...
//				printf ("{%s, %d} ", names[j], d->dcb->file_blocks[j]);
				++count;
			}
		}
		++j;
...
int SimpleFS_write(FileHandle* f, void* data, int size) {
	...
	if (aux->current_block->next_block != TBA) {
		...
		// storing all the already contained data in an array
...
int SimpleFS_remove(DirectoryHandle* d, char* filename) {
	...
	if (d->sfs == NULL) return ERROR_FS_FAULT;
	if (d == NULL) return ERROR_FS_FAULT;
		...
		if (d->dcb->file_blocks[dirhandle->pos_in_block] != TBA && d->dcb->file_blocks[dirhandle->pos_in_block] < disk->header->num_blocks) {
			...
			if (snorlax == 0) {
				...
				if (strcmp(iterator->fcb.name, filename) == 0 && iterator->fcb.is_dir == FIL) {
					...
					d->dcb->file_blocks[dirhandle->pos_in_block] = TBA;
				...
				// if the block is a DIR, remove all its content
					...
					for (int j = 0; j < dirlen; ++j) {
						SimpleFS_remove(daux, dircontent[j]);
